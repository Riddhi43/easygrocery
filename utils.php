...
for ($i = 1; $i <= $count; $i++) {
	...
	$str .= "INSERT INTO orders(id, fkCustomerId, orderType, orderDate, totalAmount, shippingAddress, paymentMethod, status) VALUES ($i, $cust_id, '$order_type', '$randomDate', 0, '$shipping_address', '$payment_method', '$order_status'); <br/>";

	$t_amt = 0;
	for ($j = 1; $j <= $num_prods; $j++) {
		$prod_id = rand(1, 80);
		//$unit_price = GetXFromYID("SELECT productPrice from product WHERE id = $prod_id");
		$sale_price = GetXFromYID("SELECT salePrice from product WHERE id = $prod_id");
		if ($sale_price) {
