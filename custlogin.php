		...
		elseif ($username == '')
			...
			if (sql_num_rows($r)) {
				...
				if ($u_password == md5($password)) {
					...
					$_SESSION[CU_SESSION_ID]->cust_cart_total = GetXFromYID("SELECT SUM(IFNULL(p.salePrice, p.productPrice) * c.qty) AS totalCartValue FROM customer_cart c LEFT JOIN product p ON c.fkProductId = p.id WHERE c.fkCustomerId = $u_id");
